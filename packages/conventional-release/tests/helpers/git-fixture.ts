...
function readScenario ({
  ...
  const root = join(FIXTURES_ROOT, useCase, fixture)
...
export function applyGitFixture ({
  cwd,
  useCase,
  fixture,
}: {
  ...
  const { root, scenario } = readScenario({ useCase, fixture })

  for (const [index, step] of scenario.steps.entries()) {
    if (!step.patch || !step.message) {
      throw new Error(`Fixture step ${index + 1} is invalid in ${useCase}/${fixture}`)
    }
